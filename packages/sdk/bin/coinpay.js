...
${colors.cyan}Commands:${colors.reset}
  ...
  ${colors.bright}reputation${colors.reset}
    ...
    did setup             Use existing DID or create one automatically
  ...
  switch (subcommand) {
    ...
    case 'did': {
      ...
      if (didSubcommand === 'claim') {
        ...
        // Ensure the merchant has a payable DID, hands-free: use the existing
        // one if there is one, otherwise create it automatically. No prompts —
        // worker onboarding (c0mpute worker register) must never block.
        ...
        try {
          ...
          existing = null; // none yet — create below
        }

        const result = existing?.did ? existing : await claimDid(client);

        if (result.did) {
          print.success(`${existing?.did ? 'Using existing DID' : 'Created DID'}: ${result.did}`);
          console.log(`  Public Key: ${result.public_key}`);
          console.log(`  Verified: ${result.verified}`);
        } else {
          print.error(result.error || 'Failed to set up DID');
          process.exitCode = 1;
        }
