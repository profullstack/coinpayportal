...
/** Wallet unlocked, with a chain holding two addresses. */
function stubWallet() {
    ...
    switch (req.type) {
      ...
      case 'listAddresses':
        return {
          ok: true,
          addresses: [
            { chain: 'ETH', index: 0, address: '0xabc', tokens: ['USDC'] },
            // A second address on the same chain — the web wallet issues these
            // on request, and funds routinely sit on the later ones.
            { chain: 'ETH', index: 1, address: '0xdef', tokens: ['USDC'] },
            { chain: 'BTC', index: 0, address: '1abc', tokens: [] },
          ],
        };
      case 'addAddress':
        return {
          ok: true,
          address: { chain: req.chain, index: 1, address: 'new-address', tokens: [] },
          addresses: [],
        };
      case 'getSettings':
...
describe('addresses per chain, the way the web wallet models them', () => {
  const rowFor = (asset: string) =>
    [...document.querySelectorAll('.account')].find(
      (row) => row.querySelector('.chain')!.textContent === asset,
    )!;

  it('lists every address a chain holds, not just the first', async () => {
    await settle();
    const addrs = [...rowFor('ETH').querySelectorAll('.addr')].map((el) => el.textContent);

    // Both ETH addresses appear. Showing only index 0 is how a balance on a
    // later address became invisible.
    expect(addrs).toEqual(['0xabc', '0xdef']);
  });

  it('numbers them so they can be told apart', async () => {
    await settle();
    const labels = [...rowFor('ETH').querySelectorAll('.addr-row .tokens')].map((el) => el.textContent);
    expect(labels).toEqual(['#1', '#2']);
  });

  it('offers another address for the chain', async () => {
    await settle();
    [...rowFor('ETH').querySelectorAll('button')]
      .find((b) => b.textContent === '+ Address')!
      .click();
    await settle();

    expect(call).toHaveBeenCalledWith({ type: 'addAddress', chain: 'ETH' });
  });

  it('has no account switcher at all', async () => {
    await settle();
    // Accounts were a MetaMask concept the portal does not have.
    expect(document.querySelector('.acct-bar')).toBeNull();
