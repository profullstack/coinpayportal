...
import { normalizeRecipients, sendBooksEmail, type SendOutcome } from './emailing';
...
export async function sendDigest(schedule: EmailScheduleRow, now: Date = new Date()): Promise<SendOutcome & { rows: number; unreviewed: number }> {
  ...
  const outcome = await sendBooksEmail({
    ...
    message: `Your ${schedule.scope} books for the seven days ${from} to ${addDays(today, -1)}. Review anything still unconfirmed at /finances/books.`,
  });
  return outcome;
}
