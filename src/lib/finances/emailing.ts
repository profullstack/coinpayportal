...
/** The books totals and review counts as an email block, for a digest that carries the pack beside the report. */
export function booksEmailBlock(summary: BooksSummary, label: string): string {
  return `<h3 style="font-size:14px;margin:18px 0 6px">Books for ${escapeHtml(label)} (${escapeHtml(summary.scope)})</h3>
...
<p style="margin:6px 0 0;font-size:12px;color:#555">${summary.rows} rows, ${summary.unreviewed} not yet reviewed, ${summary.uncategorized} uncategorised. Tax categories are a bookkeeping mapping prepared for an accountant, not tax advice.</p>`;
}
...
export async function sendReportEmail(input: {
  ...
  /** Replaces the default "CoinPay activity report" subject prefix. */
  subjectPrefix?: string;
  /** Files sent beside the report's own artifacts, within the same size cap. */
  extraAttachments?: EmailAttachment[];
  /** Email HTML appended after the report's summary, already escaped. */
  extraBody?: string;
}): Promise<SendOutcome> {
  ...
  if (input.attach !== false) {
    ...
    for (const extra of input.extraAttachments ?? []) {
      if (total + extra.content.length > MAX_ATTACHMENT_BYTES) break;
      total += extra.content.length;
      attachments.push(extra);
      attached.push(extra.filename);
    }
  }
...
${input.extraBody ?? ''}`;
  const subject = `${input.subjectPrefix ?? 'CoinPay activity report'}: ${report.period_label}${report.period_to_date ? ' (to date)' : ''}`;
